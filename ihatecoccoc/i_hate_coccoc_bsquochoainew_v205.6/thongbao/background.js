$(function(){
	...
	lichquayiframe = '<iframe src="https://www.google.com/calendar/embed?showTitle=0&amp;showNav=0&amp;showPrint=0&amp;showTz=0&amp;mode=AGENDA&amp;height=600&amp;wkst=1&amp;bgcolor=%23FFFFFF&amp;src=bsquochoainew%40gmail.com&amp;color=%232952A3&amp;ctz=Asia%2FSaigon" style=" border-width:0 " width="500" height="600" frameborder="0" scrolling="no"></iframe>'
	
	$("#caidatgiongnoi-tab").trigger("click")
	...
	$('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
		 ...
		 if ( tabid == "lichquay") {
			...
			chrome.tabs.create({url: chrome.extension.getURL('nhungtudahoc.html')});
		 } else if (tabid == "liveshowchemgio"){
			if ( $("#liveshowchemgio iframe").attr("src") == '' ){
				$("#liveshowchemgio iframe").attr("src","https://docs.google.com/forms/d/1hFLXJuxp93m-pImtxX38BQOGng1J09TiXfokAh3XJus/viewform?embedded=true")
			}
		 }
		 else {
...
/* 	if (localStorage.cddoc == "cddocgt"){
	...
	//Nhận thông báo
	$.ajax({
	  url: "https://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20xml%20where%20url%3D%27http%3A%2F%2Fwww.bsquochoai.ga%2Ffeeds%2Fposts%2Fdefault%2F-%2FTh%25C3%25B4ng%2520b%25C3%25A1o%2520i%2520hate%2520c%25E1%25BB%2591c%2520c%25E1%25BB%2591c%3Fmax-results%3D1%26orderby%3Dpublished%27&format=json&diagnostics=true&callback=",
	  cache: false
	}).success(function( re ) {
		tb = re.query.results.feed.entry
		ct = tb.content.content.split("*$*")
		if(tb.title.content == "Đã có phiên bản mới"){
			$("#caidatgiongnoi").prepend('<div><div class="alert alert-success" role="alert" style="padding:5px; margin-top:10px; margin-bottom:0px">  <span class="glyphicon glyphicon-exclamation-sign" ria-hidden="true"></span> <strong>'+tb.title.content+'</strong>  '+ct[0]+'</div>')
		} else {
			$("#caidatgiongnoi").prepend('<div><div class="alert alert-info" role="alert" style="padding:5px; margin-top:10px; margin-bottom:0px">  <span class="glyphicon glyphicon-exclamation-sign" ria-hidden="true"></span> <strong>'+tb.title.content+'</strong>  '+ct[0]+'</div>')
		}
	  });
...
if(localStorage.cddoc =="cddocgt"){
 ...
 $('[data-toggle="tooltip"]').tooltip()
  ...
  $("#tratunhanh select").change(function(){
		var l = $(this).find("option:selected")
		localStorage.setItem("cdrechuot", l.val() )
  })
  $("#gtdocngonngu select").val(localStorage.gtdocngonngu)
  $("#gttudien select").val(localStorage.gttudien)
  $("#tratunhanh select").val(localStorage.cdrechuot)
})
