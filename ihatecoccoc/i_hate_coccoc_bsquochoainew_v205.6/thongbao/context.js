...
var listmenucd=["cdluutu","cdnghedoc","cdhighlight","cdhientudien","cddoctiengviet", "cdrechuot"]
...
function onClickHandler(info, tab) {
  if (info.menuItemId == "cddocgt" || info.menuItemId == "cddocchuan" || info.menuItemId == "cdtathet" ){
	...
	} else if (info.menuItemId == "cdluutu" || info.menuItemId == "cdnghedoc" || info.menuItemId == "cdhighlight" || info.menuItemId == "cdhientudien" || info.menuItemId == "cddoctiengviet" || info.menuItemId == "cdrechuot") {
		if (info.checked) { localStorage.setItem( info.menuItemId, "1") }
		else { localStorage.setItem( info.menuItemId, "0") }
	}
	caidatlaicontextmenu()
};
...
{"title": "Thầy bá đạo (Rê chuột và ấn Ctrl)?", "type": "checkbox", "id": "cdrechuot",  "contexts":["all"]});
chrome.contextMenus.create(
...
chrome.notifications.create("", {
		...
		message:"Hãy sử dụng phần mềm của thầy với mục đích học tập, đừng sử dụng chỉ vì muốn có nó như người khác."
	},function(){})